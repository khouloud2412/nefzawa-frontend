...
export default function NewsCarousel({
  ...
  const displayArticles =
    originalArticles.length > 0
      ? [
          ...originalArticles.slice(-3),
          ...originalArticles,
          ...originalArticles.slice(0, 3),
        ]
      : [];
  ...
  if (loading)
    return (
      <div className="py-20 flex justify-center">
        <Loader2 className="animate-spin text-[#E00A0A]" size={40} />
      </div>
    );
      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-10">
          ...
          <div className="flex gap-2">
            <button
              onClick={prevSlide}
              className="p-2 rounded-full border hover:bg-[#E00A0A] hover:text-white transition-all shadow-sm"
            >
              ...
              <div key={idx} className="w-full md:w-1/3 flex-shrink-0 px-3">
                <div className="bg-[#ffeeee] rounded-[2.5rem] border h-full flex flex-col group hover:shadow-xl transition-all duration-300">
                  <div className="relative h-52 overflow-hidden rounded-t-[2.5rem]">
                    ...
                    />
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-bold text-[#E00A0A]">
                      {article.category}
                    </div>
                  </div>
                  <div className="p-6 flex flex-col flex-grow text-right">
                    ...
                    <h4 className="text-xl font-bold text-slate-900 font-arabic leading-snug line-clamp-2 mb-6">
                      {article.title}
                    </h4>
                    <Link
                      to={`/article/${cleanURL(article.category)}/${article.slug}`}
                      className="mt-auto inline-flex items-center justify-center w-full py-3 bg-white border text-slate-700 rounded-2xl font-bold text-sm hover:bg-[#E00A0A] hover:text-white transition-all duration-300"
                    >
                      {btnText}
                    </Link>
                  </div>
